...
Since the load balancer itself becomes a single choke point, it's normal to run more than one, in [[Availability Patterns | active-passive]] or [[Availability Patterns | active-active]] setup, so its own failure doesn't take everything down.
...
**Layer 4** — decides using [[OSI | transport-layer]] info only: source/destination IP and port. It never looks at the payload, so it mostly just forwards packets and rewrites addresses (NAT) — cheap and fast, but blind to what the request actually is.

**Layer 7** — decides using [[OSI | application-layer]] content: headers, cookies, the request itself. This means it can terminate the connection, actually read the request, and *then* pick a backend — e.g. sending video traffic to servers built for it while routing billing traffic to a more locked-down pool. More flexible, costlier per request, though the gap matters less on modern hardware.
